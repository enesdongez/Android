...
public class NotlarHandler extends AbstractHandler
{
    ...
    public static Map<String, String> getQueryParameters(HttpServletRequest request) {
		...
		try {
			queryString = URLDecoder.decode(queryString, "UTF-8");//Türkçe karakter onayý!
                                                      ...
                                                      ServletException
    {
        ...
        if (target.equalsIgnoreCase("/test")) {
        	sqliteClient.selectAll();
        }
        else if (target.equalsIgnoreCase("/test1")) {
        	//sqliteClient.selectAll();
        	String sql = "select id, name, password ,lastname, isim, date from kisiler where 1=1";
        	ArrayList<String[]> rows = sqliteClient.query(sql);
